...
function getApp(name = index_esm_DEFAULT_ENTRY_NAME) {
    ...
    if (!app && name === index_esm_DEFAULT_ENTRY_NAME && getDefaultAppConfig()) {
...
const dist_esm_index_esm_name = "@firebase/installations";
const esm_index_esm_version = "0.6.21";
...
const PACKAGE_VERSION = `w:${esm_index_esm_version}`;
...
const index_esm_ERROR_FACTORY = new ErrorFactory(SERVICE, SERVICE_NAME, ERROR_DESCRIPTION_MAP);
...
async function getErrorFromResponse(requestName, response) {
    ...
    return index_esm_ERROR_FACTORY.create("request-failed" /* ErrorCode.REQUEST_FAILED */, {
        requestName,
...
let index_esm_dbPromise = null;
function index_esm_getDbPromise() {
    if (!index_esm_dbPromise) {
        index_esm_dbPromise = openDB(DATABASE_NAME, DATABASE_VERSION, {
            upgrade: (db, oldVersion) => {
                // We don't use 'break' in this switch statement, the fall-through
                // behavior is what we want, because if there are multiple versions between
                // the old version and the current version, we want ALL the migrations
...
async function set(appConfig, value) {
    const key = getKey(appConfig);
    const db = await index_esm_getDbPromise();
...
function triggerRegistrationIfNecessary(installations, installationEntry) {
    if (installationEntry.registrationStatus === 0 /* RequestStatus.NOT_STARTED */) {
        if (!navigator.onLine) {
            ...
            const registrationPromiseWithError = Promise.reject(index_esm_ERROR_FACTORY.create("app-offline" /* ErrorCode.APP_OFFLINE */));
            return {
                installationEntry,
...
function updateInstallationRequest(appConfig) {
    return update(appConfig, oldEntry => {
        if (!oldEntry) {
            throw index_esm_ERROR_FACTORY.create("installation-not-found" /* ErrorCode.INSTALLATION_NOT_FOUND */);
        }
...
async function refreshAuthToken(installations, forceRefresh = false) {
    ...
    const entry = await update(installations.appConfig, oldEntry => {
        if (!isEntryRegistered(oldEntry)) {
            throw index_esm_ERROR_FACTORY.create("not-registered" /* ErrorCode.NOT_REGISTERED */);
        }
        ...
        if (!forceRefresh && isAuthTokenValid(oldAuthToken)) {
            ...
            if (!navigator.onLine) {
                throw index_esm_ERROR_FACTORY.create("app-offline" /* ErrorCode.APP_OFFLINE */);
            }
...
async function deleteInstallations(installations) {
    ...
    if (entry) {
        if (entry.registrationStatus === 1 /* RequestStatus.IN_PROGRESS */) {
            ...
            throw index_esm_ERROR_FACTORY.create("delete-pending-registration" /* ErrorCode.DELETE_PENDING_REGISTRATION */);
        }
 ...
 * @public
 */
function getInstallations(app = index_esm_getApp()) {
    const installationsImpl = index_esm_getProvider(app, 'installations').getImmediate();
...
function getMissingValueError(valueName) {
    return index_esm_ERROR_FACTORY.create("missing-app-config-values" /* ErrorCode.MISSING_APP_CONFIG_VALUES */, {
        valueName
    });
}
...
registerVersion(dist_esm_index_esm_name, esm_index_esm_version);
// BUILD_TARGET will be replaced by values like esm, cjs, etc during the compilation
registerVersion(dist_esm_index_esm_name, esm_index_esm_version, 'esm2020');
 ...
 * limitations under the License.
 */
const index_esm_logger = new Logger('@firebase/analytics');
...
const esm_index_esm_ERROR_FACTORY = new ErrorFactory('analytics', 'Analytics', index_esm_ERRORS);
...
function createGtagTrustedTypesScriptURL(url) {
    if (!url.startsWith(GTAG_URL)) {
        const err = esm_index_esm_ERROR_FACTORY.create("invalid-gtag-resource" /* AnalyticsError.INVALID_GTAG_RESOURCE */, {
            gtagURL: url
        });
        index_esm_logger.warn(err.message);
        return '';
    }
    return url;
}
...
async function fetchDynamicConfig(appFields) {
    ...
    if (response.status !== 200 && response.status !== 304) {
        ...
        throw esm_index_esm_ERROR_FACTORY.create("config-fetch-failed" /* AnalyticsError.CONFIG_FETCH_FAILED */, {
...
retryData = defaultRetryData, timeoutMillis) {
    ...
    if (!appId) {
        throw esm_index_esm_ERROR_FACTORY.create("no-app-id" /* AnalyticsError.NO_APP_ID */);
    }
    if (!apiKey) {
        if (measurementId) {
            return {
        ...
        throw esm_index_esm_ERROR_FACTORY.create("no-api-key" /* AnalyticsError.NO_API_KEY */);
    }
...
async function attemptFetchDynamicConfigWithRetry(appFields, { throttleEndTimeMillis, backoffCount }, signal, retryData = defaultRetryData // for testing
    ...
    try {
        ...
        if (measurementId) {
            index_esm_logger.warn(`Timed out fetching this Firebase app's measurement ID from the server.` +
    ...
    try {
        ...
        if (!isRetriableError(error)) {
            retryData.deleteThrottleMetadata(appId);
            if (measurementId) {
                index_esm_logger.warn(`Failed to fetch this Firebase app's measurement ID from the server.` +
                    ` Falling back to the measurement ID ${measurementId}` +
        ...
        index_esm_logger.debug(`Calling attemptFetch again in ${backoffMillis} millis`);
...
function setAbortableTimeout(signal, throttleEndTimeMillis) {
    return new Promise((resolve, reject) => {
        ...
        signal.addEventListener(() => {
            ...
            reject(esm_index_esm_ERROR_FACTORY.create("fetch-throttle" /* AnalyticsError.FETCH_THROTTLE */, {
...
async function internalGetGoogleAnalyticsClientId(gtagFunction, initializationPromise) {
    ...
    return new Promise((resolve, reject) => {
        gtagFunction("get" /* GtagCommand.GET */, measurementId, 'client_id', (clientId) => {
            if (!clientId) {
                reject(esm_index_esm_ERROR_FACTORY.create("no-client-id" /* AnalyticsError.NO_CLIENT_ID */));
            }
            resolve(clientId);
        });
    });
...
async function validateIndexedDB() {
    if (!isIndexedDBAvailable()) {
        index_esm_logger.warn(esm_index_esm_ERROR_FACTORY.create("indexeddb-unavailable" /* AnalyticsError.INDEXEDDB_UNAVAILABLE */, {
...
async function _initializeAnalytics(app, dynamicConfigPromisesList, measurementIdToAppId, installations, gtagCore, dataLayerName, options) {
    ...
    dynamicConfigPromise
        ...
        if (app.options.measurementId &&
            ...
            index_esm_logger.warn(`The measurement ID in the local Firebase config (${app.options.measurementId})` +
        ...
        .catch(e => index_esm_logger.error(e));
...
function settings(options) {
    if (globalInitDone) {
        throw esm_index_esm_ERROR_FACTORY.create("already-initialized" /* AnalyticsError.ALREADY_INITIALIZED */);
    }
...
function warnOnBrowserContextMismatch() {
    ...
    if (mismatchedEnvMessages.length > 0) {
        const details = mismatchedEnvMessages
            ...
            .join(' ');
        const err = esm_index_esm_ERROR_FACTORY.create("invalid-analytics-context" /* AnalyticsError.INVALID_ANALYTICS_CONTEXT */, {
            errorInfo: details
        });
        index_esm_logger.warn(err.message);
    }
...
function factory(app, installations, options) {
    ...
    if (!app.options.apiKey) {
        if (app.options.measurementId) {
            index_esm_logger.warn(`The "apiKey" field is empty in the local Firebase config. This is needed to fetch the latest` +
    ...
    if (initializationPromisesMap[appId] != null) {
        throw esm_index_esm_ERROR_FACTORY.create("already-exists" /* AnalyticsError.ALREADY_EXISTS */, {
            id: appId
        });
    }
    if (!globalInitDone) {
...
function setCurrentScreen(analyticsInstance, screenName, options) {
    analyticsInstance = index_esm_getModularInstance(analyticsInstance);
    setCurrentScreen$1(wrappedGtagFunction, initializationPromisesMap[analyticsInstance.app.options.appId], screenName, options).catch(e => index_esm_logger.error(e));
}
/**
 * Retrieves a unique Google Analytics identifier for the web client.
 * See {@link https://developers.google.com/analytics/devguides/collection/ga4/reference/config#client_id | client_id}.
...
function setUserId(analyticsInstance, id, options) {
    analyticsInstance = index_esm_getModularInstance(analyticsInstance);
    setUserId$1(wrappedGtagFunction, initializationPromisesMap[analyticsInstance.app.options.appId], id, options).catch(e => index_esm_logger.error(e));
}
/**
...
function setUserProperties(analyticsInstance, properties, options) {
    ...
    setUserProperties$1(wrappedGtagFunction, initializationPromisesMap[analyticsInstance.app.options.appId], properties, options).catch(e => index_esm_logger.error(e));
}
/**
...
function setAnalyticsCollectionEnabled(analyticsInstance, enabled) {
    analyticsInstance = index_esm_getModularInstance(analyticsInstance);
    setAnalyticsCollectionEnabled$1(initializationPromisesMap[analyticsInstance.app.options.appId], enabled).catch(e => index_esm_logger.error(e));
}
/**
...
function logEvent(analyticsInstance, eventName, eventParams, options) {
    analyticsInstance = getModularInstance(analyticsInstance);
    logEvent$1(wrappedGtagFunction, initializationPromisesMap[analyticsInstance.app.options.appId], eventName, eventParams, options).catch(e => index_esm_logger.error(e));
}
/**
...
const analytics_dist_esm_index_esm_name = "@firebase/analytics";
const dist_esm_index_esm_version = "0.10.21";
...
function registerAnalytics() {
    ...
    registerVersion(analytics_dist_esm_index_esm_name, dist_esm_index_esm_version);
    // BUILD_TARGET will be replaced by values like esm, cjs, etc during the compilation
    registerVersion(analytics_dist_esm_index_esm_name, dist_esm_index_esm_version, 'esm2020');
    function internalFactory(container) {
        try {
            ...
            throw esm_index_esm_ERROR_FACTORY.create("interop-component-reg-failed" /* AnalyticsError.INTEROP_COMPONENT_REG_FAILED */, {
                reason: e
            });
        }
    }
